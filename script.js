...
const play = {
    ...
    dead: 0,
    set updateData(data) {
        this[data] += 1; // обращение к полю data (должна быть строкой)
        this.render();
    },
    render() {
        ...
        if (index >= 0) {
            ...
            if (life < 0) {
                ...
                for (let i = 0; i < ship.location.length; i++) {
                    const startCoordX = ship.location[i][0] - 1;
                    for (let j = startCoordX; j < startCoordX + 3; j++) {
                        const startCoordY = ship.location[i][1] - 1;
                        for (let z = startCoordY; z < startCoordY + 3; z++) {
                            if (j >= 0 && j < 10 && z >= 0 && z < 10 && ship.location.indexOf(j + '' + z) < 0) {
                                const id = j + '' + z;
